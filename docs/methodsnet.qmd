---
title: "Accounting for Heterogeneity in Meta-Analysis"
...
1. 20 minutes: Basic Models for Meta-Analysis
2. 10 minutes: Q&A
3. 30 minutes: Exercises
4. 20 minutes: Advanced Methods for Dealing with Heterogeneity
5. 10 minutes: Q&A
6. 30 minutes: Exercises

<!-- * Each 90-minute session begins with a 20 minute introductory lecture, followed by 10 minutes of Q&A. Then follow two 30 minute exercise blocks, guided by an online coursebook with instructions, code, and formative self-assessment questions. The workshop chair walks around to support students. The last 10 minutes of each exercise block consist of discussion and peer feedback to resolve any sticking points. -->

<!-- Presentations: The first 30-minute presentation covers the basic models for meta-analysis (fixed-effect and random effects analysis), meta-regression, and Bayesian penalized meta-regression. The second 30-minute presentation covers random forest meta-analysis and the Product Bayes Factor. -->

<!-- Moderation approach: After the exercise blocks, the workshop chair facilitates knowledge exchange among participants so they resolve each other's sticking points. Only as a last resort does the chair step in to provide answers. -->
<!-- Convenor's Biographies: Dr. Caspar J. Van Lissa is associate professor of social data science at Tilburg University, the Netherlands and associate editor for Research Synthesis Methods. He has developed machine learning-based evidence synthesis methods for dealing with heterogeneity in meta-analysis, combining evidence from heterogeneous studies, and qualitative research synthesis methods using text mining. He serves as board member of Tilburg Young Academy and chair of the Tilburg Open Science Community. -->



## Conceptual Understanding {.smaller}
...
<font color="red">Not covered:</font>Individual participant data meta-analysis
...
<!-- * 1950: Eysenk concludes psychotherapy does not work -->
...
Popularity grows exponentially [@ioannidisMassProductionRedundant2016]
...
3. Test hypotheses
4. Objectively synthesize literature
5. Identify knowledge gaps and future directions
6. Power analysis for future studies
7. **Identify effect heterogeneity**
8. **Explain effect heterogeneity**
...
Assign more weight to more informative studies

* More participants
* Lower sampling error

This is reflected in the standard error

## Effect sizes: Standard error {.smaller}
...
* $SE^2$ is the sampling variance
* A measure of precision / uncertainty about the sample statistic as estimator of the population value
...
```{=html}
<iframe width="1000" height="700" src="https://rpsychologist.com/d3/CI/" title="Sampling Distribution"></iframe>
```

## Effect sizes: Sampling distribution

```{=html}
<iframe width="1000" height="700" src="https://utrecht-university.shinyapps.io/cjvanlissa_sampling_distribution" title="Sampling Distribution"></iframe>
```
...
<!-- Meta-analysis is **not** immune to small sample problems (small N, many moderators p) -->
...
## Heterogeneity statistics: I2 {.smaller}
...
2. **If** that test is not-significant, then use a fixed-effect model
...
## Dummies in meta-regression {.smaller}
...
1. Studies are incomparable:
    - Do not pool at all (apples / oranges)
    - **NEW: aggregate evidence using *Product Bayes Factor***
2. (Close) replications: fixed effect model
3. Conceptual replications with minor differences: random effects model
4. Known systematic differences: Explain heterogeneity
    - Meta-regression
5. Unknown systematic differences: *Explore heterogeneity*
...
```{=html}
<iframe width="1000" height="700" src="https://cjvanlissa.shinyapps.io/Polynomials/" title="Polynomials"></iframe>
```
...
$$
T_i = \beta_0 + \beta_1x_1 + \ldots + \beta_px_p + \zeta_i + \epsilon_i
$$
Typically estimated using REML
...
fit_lasso <- brma(yi ~ ., data = bonapersona)
```
```{r echo = FALSE, eval = TRUE}
fit_lasso <- readRDS("brma_results.RData")
print(summary(fit_lasso))
```
...
**Method reference: [Van Lissa, van Erp, & Clapper (2023). Selecting relevant moderators with Bayesian regularized meta‐regression. Research Synthesis Methods, 14(2), 301-322.](https://onlinelibrary.wiley.com/doi/abs/10.1002/jrsm.1628)**

[Kwee et al. (2023). Anxiolytic effects of endocannabinoid enhancing compounds: A systematic review and meta-analysis. European Neuropsychopharmacology, 72, 79-94.](https://www.sciencedirect.com/science/article/pii/S0924977X23000676)

[Heesbeen et al. (2023). The effect of SSRIs on fear learning: a systematic review and meta-analysis. Psychopharmacology, 240(11), 2335-2359.](https://link.springer.com/article/10.1007/s00213-023-06333-7)
...
[Curry et al., 2018. Happy to help? A systematic review and meta-analysis of the effects of performing acts of kindness on the well-being of the actor. Journal of Experimental Social Psychology, 76, 320-329.](https://www.sciencedirect.com/science/article/pii/S0022103117303451)

[Terrer et al., 2019. Nitrogen and phosphorus constrain the CO2 fertilization of global plant biomass. Nature Climate Change, 9(9), 684-689.](https://www.nature.com/articles/s41558-019-0545-2)

[Roberts et al., 2019. The effect of short-term exposure to the natural environment on depressive mood: A systematic review and meta-analysis. Environmental research, 177, 108606.](https://www.sciencedirect.com/science/article/pii/S0013935119304037)

[Bonapersona et al., 2019. The behavioral phenotype of early life adversity: A 3-level meta-analysis of rodent studies. Neuroscience & Biobehavioral Reviews, 102, 299-307.](https://www.sciencedirect.com/science/article/pii/S014976341930168X)
...
library(bain)
set.seed(1)
res <- pbf(yi = kuiper2013$beta,
    vi = kuiper2013$vi,
    ni = kuiper2013$n,
hypothesis = "y = 0; y > 0; y < 0")
saveRDS(res, "output_pbf.RData")
```
...
```
```{r echo=FALSE, eval = TRUE}
res <- readRDS("output_pbf.RData")
format.data.frame(res, digits = 2)
...
[Van Lissa, Kuiper, & Clapper (in print). Aggregating evidence from conceptual replication studies using the product Bayes factor. Research Synthesis Methods](https://osf.io/preprints/psyarxiv/nvqpw/download)

[Van Lissa et al. (2021). Teacher’s corner: Evaluating informative hypotheses using the Bayes factor in structural equation models. Structural Equation Modeling: A Multidisciplinary Journal, 28(2), 292-301.](https://www.tandfonline.com/doi/pdf/10.1080/10705511.2020.1745644)

**Example paper:**

[Van Leeuwen et al. (2024). Morality as Cooperation, Politics as Conflict. Social Psychological Bulletin, 19, 1-22.](https://osf.io/preprints/psyarxiv/wm6rk/download)
